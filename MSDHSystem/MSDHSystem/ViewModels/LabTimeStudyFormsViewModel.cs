        ...
        public void Submit()
        {
            ...
            for (int i = 0; i < TimeStudyItems.Count; i++)
            {
                ...
                else
                {
                    ...
                    strQuery = string.Format("INSERT INTO TimeStudyEmmployeeInfo(EmployeeName, PIN, Classification, ORG, PID_NO, CalenderWeek, FormType) VALUES('{0}', '{1}', '{2}', '{3}', '{4}', '{5}', 'LTS')", Xamarin.Essentials.SecureStorage.GetAsync("username").Result, Xamarin.Essentials.SecureStorage.GetAsync("pin_number").Result, Location + "/" + Jobname, Orgcode, Xamarin.Essentials.SecureStorage.GetAsync("pid_number").Result, GetWeekNumber(startDate).ToString());
                    command = new SqlCommand(strQuery, con);
                    command.ExecuteNonQuery();
